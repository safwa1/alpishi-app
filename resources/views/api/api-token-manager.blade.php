<div>
    ...
    <x-jet-form-section submit="createApiToken">
        ...
        <x-slot name="description">
            {{ __('تسمح رموز API لخدمات الجهات الخارجية بالمصادقة باستخدام تطبيقنا نيابة عنك.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="name" value="{{ __('إسم الـ Token') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-jet-label for="permissions" value="{{ __('الصلاحيات') }}" />

                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                <x-jet-checkbox wire:model.defer="createApiTokenForm.permissions" :value="$permission"/>
                                <span class="mr-2 text-sm text-gray-600">{{ \App\Utils\PermissionTranslator::toArabic($permission) }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('إدارة الـ API Tokens') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('يمكنك حذف أي من الرموز المميزة الحالية الخاصة بك إذا لم تعد هناك حاجة إليها.') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('أخر إستخدام') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
