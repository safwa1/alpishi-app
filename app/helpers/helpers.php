...
if (!function_exists('to_html_currency')) {
    ...
    function to_html_currency(string|int|float $input, ?string $prefix = null, string $prefixTag = 'span'): string
    {
        $result = to_currency($input);
        return $prefix != null ? "{$result} <{$prefixTag}>{$prefix}</{$prefixTag}>" : $result;
    }
}
